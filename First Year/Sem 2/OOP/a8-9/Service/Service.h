...
class Service {
    ...
    public:
        ...
        //Gets admin repo
        const vector<Event> &getAdminRepo();
        //Gets user repo
        vector<Event> getUserRepo();
    ...
    //link -  a link to the event
        void addServiceAdmin(string title, string description, Date date, int time, int attendance, string link);
    //deletes an event based on the title
        void deleteServiceAdmin(string title);
    ...
    //command - what to replace
        void updateEventStringAdmin(string title, string newText, int command);
    //updates the int parts of the event struct (time, attendance)
    // title - the title of the event we want to change
    //newInt - the int we replace it with
    //command - what to replace
        void updateEventIntAdmin(string title, int newInt, int command);
    //updates the Date part of the event struct
    // title - the title of the event we want to change
    // day - new day
    // month - new month
    // year - new year
        void updateEventDateAdmin(string title, int day, int month, int year);
        //
        void addServiceUser(Event e);
        void deleteServiceAUser(Event e);
