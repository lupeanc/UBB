...
void testRemove() {
	...
	b1.add(0);
	assert(b1.size() == 402);
	testIteratorSteps(b1);
	for (int i = -30; i < 30; i++) {

		assert(b1.search(i) == true);
		assert(b1.remove(i) == true);
		if (i != 0) {
			assert(b1.search(i) == false);
		}
		else {
			assert(b1.search(i) == true);
		}
...
void testQuantity() {
	//todo CHANGE QUANTITY TO LOWER VALUES BC THE PROGRAM CANT ASSIGN THIS MANY
