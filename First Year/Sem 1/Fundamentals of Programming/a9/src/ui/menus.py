...
# RENT/RETURN
    ...
    def get_return(self):
        try:
            book_id = int(input("Book ID: "))
            rental_id = int(input("Rental ID: "))
            return book_id, rental_id
        except ValueError:
            raise ValueError("The ID has to be an integer")
...
# DISPLAY
    def check_id(self, list_of_entities):
        valid_list = []

        for entity in list_of_entities:
            if entity not in valid_list:
                valid_list.append(entity)
        return valid_list


# MOST POPULAR
    def most_popuar(self):
        print("1. Books")
        print("2. Clients")
        print("3. Authors")